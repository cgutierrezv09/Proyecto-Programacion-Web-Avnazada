...
const renderizarTabla = () => {
    ...
    const registrosPagina = usuariosFiltrados.slice(inicio, fin);
    ...
    registrosPagina.forEach(registro => {
        ...
        <tr>
            ...
            <td><span class="badge bg-primary">${registro.rol}</span></td>
...
const renderizarPaginacion = () => {
    ...
    const totalPaginas = Math.ceil(usuariosFiltrados.length / REGISTROS_POR_PAGINA);
...
const eliminarModal = (registro) => {
    ...
    modalBody.innerHTML = `
        <p class="mb-2"><strong>¿Deseas eliminar este Usuario?</strong></p>
...
const mostrarModalEliminar = (registro) => {
    ...
    btnEliminar.addEventListener('click', function () {
        usuarios = usuarios.filter(r => r.id !== registro.id);
        usuariosFiltrados = usuariosFiltrados.filter(r => r.id !== registro.id);
